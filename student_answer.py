...
class Terminal:
    ...
    def is_cross_has_win(self):
        ...
        right_check = all([right == self.board[i][len(self.board)-1-i] and right != '.' for i in range(1, len(self.board))])  # range(start, end) end not call
...
|..X|
+---+"""
# test_ = Terminal([['X', 'O', 'O'], ['.', 'O', '.'], ['.', 'O', '.']])
# test_.is_terminal()
# print(test_.winner_player)
